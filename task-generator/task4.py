...
def generate_matrix_system(A_size, B_size, C_size, range):
    ...
    B = np.random.uniform(range[0], range[1], B_size)
    C = np.random.uniform(range[0], range[1], C_size)
    return A, B, C
...
def gen_task_4(A_size, B_size, C_size, range):
    ...
    A_inv = matrix_to_word1(np.linalg.inv(rat_A)).replace("//", "/").replace("/1 ", " ")
    B_inv = matrix_to_word1(np.linalg.inv(rat_B)).replace("//", "/").replace("/1 ", " ")

    X = matrix_to_word1(np.linalg.inv(rat_A) @ rat_C @ np.linalg.inv(rat_B)).replace("//", "/").replace("/1 ", " ")
